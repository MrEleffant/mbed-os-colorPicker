...
int main() {
  ...
  while (true) {
    if (joystick.Ytriggered() == JoystickDirection::LeftTrigg) {
      std::cout << "Selecting prev color : " << guessColor.previousColor()
                << std::endl;
    } else if (joystick.Ytriggered() == JoystickDirection::RightTrigg) {
      std::cout << "Selecting next color : " << guessColor.nextColor()
                << std::endl;
    }

    float coeff = joystick.Xtriggered();
    if (coeff != 0.0) {
      std::cout << "Changing color value : " << guessColor.moveColor(coeff)
                << std::endl;
    }
